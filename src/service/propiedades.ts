...
export async function getPropiedades(): Promise<PropiedadListItem[]> {
  ...
  if (Array.isArray(payload)) return payload as PropiedadListItem[];
  if (payload && Array.isArray(payload.data)) return payload.data as PropiedadListItem[];
...
// Use central MOCK_BUILDINGS and ID generators from service_mock.ts
